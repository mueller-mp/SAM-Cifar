...
from asam_pyramid import ASAM, SAM, ExtraASAM, ExtraSAM
...
def train(args):
    ...
    for epoch in range(args.epochs):
        ...
        for idx, (inputs, targets) in enumerate(train_loader):
            ...
            grads_new = [p.grad.data.clone().detach() for p in model.parameters() if p.grad is not None]
            if idx==0 and epoch==0:
                grads_old = [p.grad.data.clone().detach() for p in model.parameters() if p.grad is not None]
                grads_adv = [p.grad.data.clone().detach() for p in model.parameters() if p.grad is not None]

            # angle between grad L(w_t) and grad L(w_t+1)
            cosine_old_new = angle_between_grads(grads_old, grads_new)
            writer.add_scalar('train/cosine_old_new', cosine_old_new,
                              global_step=epoch * len(train_loader) + idx)
            # angle between grad L(w_t) and grad L(w_t + adv)
            cosine_old_adv = angle_between_grads(grads_old, grads_adv)
            writer.add_scalar('train/cosine_old_adv', cosine_old_adv,
                              global_step=epoch * len(train_loader) + idx)
            # angle between grad L(w_t+1) and grad L(w_t + adv)
            cosine_new_adv = angle_between_grads(grads_new, grads_adv)
            writer.add_scalar('train/cosine_new_adv', cosine_new_adv,
                              global_step=epoch * len(train_loader) + idx)

            grads_old = [p.clone().detach() for p in grads_new]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save', default='./snapshots/track_angles', type=str, help='directory to save models in')
    ...
    assert args.minimizer in ['ASAM', 'SAM', 'SGD', 'ExtraASAM', 'ExtraSAM'], \
