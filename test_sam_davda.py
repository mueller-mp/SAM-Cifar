...
# from asam_pyramid import ASAM, SAM, ExtraASAM, ExtraSAM, SAM_BN, ASAM_BN
from sam_davda import SAM_BN
...
def train(args):
    ...
    if args.model == 'pyramid':
        ...
        model = eval(args.model)(dim=256, depth=8, kernel_size=8, patch_size=1, n_classes=100).cuda()
    ...
    bn_params = [v for k, v in all_params if ('bn' in k or 'norm' in k)]
    other_params = [v for k, v in all_params if not ('bn' in k or 'norm' in k)]
    param_groups = [{
        'params': bn_params,
        'weight_decay': 0.,
        'sam': 0 if args.no_bn else 1,
    }, {
        'params': other_params,
        'weight_decay': args.weight_decay,
        'sam': 0 if args.only_bn else 1,
    }]

    assert args.minimizer=='SAM_BN'
    base_optimizer = torch.optim.SGD
    minimizer = SAM_BN(param_groups, base_optimizer, momentum=args.momentum, rho=args.rho, adaptive=0, lr=args.lr)


    # Learning Rate Scheduler
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(minimizer if args.minimizer=='SGD' else minimizer.base_optimizer, args.epochs)
...
if __name__ == "__main__":
    ...
    assert args.minimizer in ['ASAM', 'SAM', 'SGD', 'ExtraASAM', 'ExtraSAM', 'SAM_BN', 'ASAM_BN'], \
