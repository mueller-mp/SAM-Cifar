...
from asam_pyramid import ASAM, SAM, ExtraASAM, ExtraSAM, SAM_BN, ASAM_BN, Shampoo
...
def train(args):
    ...
    '_onlyBN' if args.only_bn else '', '_noBN' if args.no_bn else '')
    ...
    # base optimizer
    if args.base_optimizer=='Shampoo':
        optimizer = Shampoo(model.parameters(), lr=args.lr, momentum = args.momentum, weight_decay = args.weight_decay)
    else:
        optimizer = torch.optim.SGD(model.parameters(), lr=args.lr,
                                momentum=args.momentum, weight_decay=args.weight_decay)
    # minimizer
    if args.minimizer in ['SGD']:
        minimizer =  torch.optim.SGD(model.parameters(), lr=args.lr,
                                momentum=args.momentum, weight_decay=args.weight_decay)
    elif args.minimizer=='Shampoo':
        minimizer = Shampoo(model.parameters(), lr=args.lr, momentum = args.momentum, weight_decay = args.weight_decay)
        ...
        minimizer = eval(args.minimizer)(optimizer, model, rho=args.rho, eta=args.eta, layerwise=args.layerwise,
                                         ...
                                         normalize_bias=args.normalize_bias, no_bn=args.no_bn, only_bn=args.only_bn,
                                         update_grad=args.update_grad)
    else:
        minimizer = eval(args.minimizer)(optimizer, model, rho=args.rho, eta=args.eta, layerwise=args.layerwise,
                                     elementwise=args.elementwise, filterwise=args.filterwise, p=args.p, normalize_bias=args.normalize_bias)

    # Learning Rate Scheduler
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(minimizer if args.minimizer in ['SGD', 'Shampoo'] else minimizer.optimizer, args.epochs)
    ...
    for epoch in range(args.epochs):
        ...
        for idx, (inputs, targets) in enumerate(train_loader):
            ...
            if 'Extra' in args.minimizer: # no forward-backward needed, since old gradient is used for ascent step
                ...
                if args.minimizer in ['SGD', 'Shampoo']:
                    minimizer.step()
                    minimizer.zero_grad()
                else:
                    minimizer.ascent_step()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--base_optimizer", default='SGD', type=str, help="SGD or Shampoo")
    ...
    parser.add_argument("--update_grad", action='store_true', help="perform ascent step only with bn layer")
    ...
    assert args.minimizer in ['ASAM', 'SAM', 'SGD', 'ExtraASAM', 'ExtraSAM', 'SAM_BN', 'ASAM_BN', 'Shampoo'], \
