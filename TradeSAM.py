...
from asam_pyramid import ASAM, SAM, SAM_mock, TradeSAM
...
def train(args):
    ...
    if n_chunks != 1:
        print(
            'Using {}-sharpness with factor {}, since batch_size={} and m={}!'.format(args.m, n_chunks, args.batch_size,
                                                                                      args.m))
    model_name = '{}_{}_{}_lr{}_bsize{}_epochs{}_rho{}_layerwise{}_filterwise{}_elementwise{}_autoaugment{}_cutmix{}_m{}_eta{}_{}_alpha{}_seed{}'.format(
                                        ...
                                        args.cutmix,
                                        args.m, args.eta, args.norm_adaptive, args.alpha, args.seed)
    ...
    if args.minimizer == 'SGD':
        minimizer = optimizer
    elif args.minimizer == 'SAM_mock':
        minimizer = eval(args.minimizer)(optimizer, model, rho=args.rho, alpha=args.alpha)
    elif args.minimizer=='TradeSAM':
        minimizer=eval(args.minimizer)(optimizer, rho=args.rho)
    else:
    ...
    for epoch in range(args.epochs):
        ...
        for idx, (inputs, targets) in enumerate(train_loader):
            ...
            if args.minimizer=='TradeSAM':
                with torch.no_grad():
                    minimizer.perturb()
                    perturbed_predictions = model(inputs)
                    minimizer.unperturb()
                predictions = model(inputs)
                batch_loss_kl = kl_criterion(F.log_softmax(perturbed_predictions, dim=1),
                                             F.softmax(predictions, dim=1))
                batch_loss = criterion(predictions, targets) + args.beta*batch_loss_kl
                batch_loss.mean().backward()
                minimizer.step()
                norm=0.
                loss_adv=0.
                batch_loss_2 = batch_loss_kl.clone().detach()
            else:
                predictions = model(inputs)
                ...
                if args.minimizer not in ['SGD', 'SAM_mock', 'TradeSAM']:
                    # Descent Step
        ...
        with torch.no_grad():
            ...
            if args.minimizer in ['SGD', 'SAM_mock']:
                loss_adv=0
...
if __name__ == "__main__":
    ...
    parser.add_argument("--alpha", default=0.0, type=float, help="gradient noise level")
    parser.add_argument("--beta", default=1.0, type=float, help="prefactor of trades loss")
    ...
    assert args.minimizer in ['ASAM', 'SAM', 'SGD', 'SAM_mock', 'TradeSAM'], \
