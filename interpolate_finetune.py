...
def train(args):
    ...
    # load model1
    model_name = '{}_{}_{}_lr{}_{}bsize{}_epochs{}_rho{}_layerwise{}_filterwise{}_elementwise{}_autoaugment{}_cutmix{}_m{}_seed{}'.format(
        args.dataset, args.model, args.model1, args.lr, 'ftepochs_{}_'.format(args.ft_epochs) if args.model1 == 'FINETUNE' else '', args.batch_size, args.epochs*2 if args.model1=='NORMAL' else args.epochs,
    ...
    state_dict = torch.load((model_to_load), map_location=torch.device('cpu'))
    model1.load_state_dict(cuda_to_cpu_statedict(state_dict))

    # load model2
    model_to_load = os.path.join(args.save,
                                    '{}_{}_{}_lr{}_{}bsize{}_epochs{}_rho{}_layerwise{}_filterwise{}_elementwise{}_autoaugment{}_cutmix{}_m{}_seed{}'.format(
                                        args.dataset, args.model,args.model2, args.lr, 'ftepochs_{}_'.format(args.ft_epochs) if args.model2 == 'FINETUNE' else '',args.batch_size, args.epochs*2 if args.model2=='NORMAL' else args.epochs,
                                        0.5, False, False, False, args.autoaugment,
                                        args.cutmix,
                                        args.m,args.seed) + '.pt')
    state_dict = torch.load(model_to_load, map_location=torch.device('cpu'))
    model2.load_state_dict(cuda_to_cpu_statedict(state_dict))

    # model_to_load = os.path.join(args.save, '{}_{}_seed{}_init'.format(args.dataset, args.model, args.seed) + '.pt')
    # state_dict = torch.load(model_to_load, map_location=torch.device('cpu'))
    # model_init.load_state_dict((state_dict))
    ...
    with open(os.path.join(args.save, model_name + '_interpolation'+'_from{}'.format(args.model1) + '_to{}'.format(args.model2) +'.csv'), 'w') as f:
    ...
    with torch.no_grad():
        for step in np.linspace(0,1,num=args.interpolation_steps):
            ...
            interpolate(model1, model2, model_inter,step)
            ...
            with open(os.path.join(args.save, model_name + '_interpolation' + '_from{}'.format(args.model1) + '_to{}'.format(
                    args.model2) + '.csv'), 'w') as f:
    ...
    plt.scatter(range(len(test_losses)),test_losses,c='blue')
    plt.legend()
    plt.savefig(os.path.join(args.save, model_name + '_interpolation'+'_from{}'.format(args.model1) + '_to{}'.format(args.model2) +'.png'))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save', default='./snapshots/basins_parallel', type=str, help='directory to save models in')
    ...
    parser.add_argument("--model1", default='SAM', type=str, choices=['SAM','NORMAL','FINETUNE'])
    parser.add_argument("--model2", default='NORMAL', type=str, choices=['SAM', 'NORMAL', 'FINETUNE'])
