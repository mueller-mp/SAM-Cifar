...
from asam_pyramid import ASAM, SAM, ExtraASAM, ExtraSAM, SAM_BN, ASAM_BN, AVG_ASAM_BN, AVG_SAM_BN, ASAM_2RHO, SAM_2RHO
...
def train(args):
    ...
    if n_chunks != 1:
        print(
            'Using {}-sharpness with factor {}, since batch_size={} and m={}!'.format(args.m, n_chunks, args.batch_size,
                                                                                      args.m))
    model_name = '{}_{}_{}_p{}_lr{}_bsize{}_epochs{}_rhoBN{}_rhoConv{}_layerwise{}_filterwise{}_elementwise{}_autoaugment{}_cutmix{}_m{}_eta{}_{}_seed{}{}{}{}{}'.format(
                                        args.dataset, args.model, args.minimizer, args.p, args.lr, args.batch_size, args.epochs,
                                        args.rho_bn, args.rho_conv, args.layerwise, args.filterwise, args.elementwise, args.autoaugment,
                                        args.cutmix,
    ...
    elif '2RHO' in args.minimizer:
        minimizer = eval(args.minimizer)(optimizer, model, rho_bn=args.rho_bn, rho_conv=args.rho_conv, eta=args.eta, layerwise=args.layerwise,
                                         elementwise=args.elementwise, filterwise=args.filterwise, p=args.p,
                                         normalize_bias=args.normalize_bias, no_bn=args.no_bn, only_bn=args.only_bn,
                                         update_grad=args.update_grad, no_grad_norm=args.no_grad_norm)
    else:
        minimizer = eval(args.minimizer)(optimizer, model, rho=args.rho, eta=args.eta, layerwise=args.layerwise,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--rho_bn",  default=1.0, type=float)
    parser.add_argument("--rho_conv",  default=0.1, type=float)
    ...
    assert args.minimizer in ['ASAM', 'SAM', 'SGD', 'ExtraASAM', 'ExtraSAM', 'SAM_BN', 'ASAM_BN', 'AVG_ASAM_BN', 'AVG_SAM_BN', 'ASAM_2RHO', 'SAM_2RHO'], \
